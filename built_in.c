...
int change_cd(info_t *info)
{
    ...
    if (!info->argv[1])
    {
        dir = get_env2(info, "HOME=");
        if (!dir)
            chdir_ret = chdir((dir = get_env2(info, "PWD=")) ? dir : "/");
        else
            chdir_ret = chdir(dir);
    }
    else if (_strcmp(info->argv[1], "-") == 0)
    {
        if (!get_env2(info, "OLDPWD="))
        {
            _puts(s);
            _putchar('\n');
            return (1);
        }
        _puts(get_env2(info, "OLDPWD=")), _putchar('\n');
        chdir_ret = chdir((dir = get_env2(info, "OLDPWD=")) ? dir : "/");
    }
    else
    ...
    {
        set_env(info, "OLDPWD", get_env2(info, "PWD="));
